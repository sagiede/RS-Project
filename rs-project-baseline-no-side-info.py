...
if __name__ == '__main__':
    ...
    x = tf.keras.layers.Dense(mid_dim, input_shape=(in_dim,), activation='tanh')(input_ratings)
    x = tf.keras.layers.Dropout(0.5)(x)
    x = tf.keras.layers.Dense(in_dim)(x)
    model = tf.keras.Model(inputs=input_ratings, outputs=x)
    ...
    for epoch in range(epochs):
        ...
        history = model.fit(corrupted, y_true_matrix_train, epochs=1, batch_size=64)
        train_losses.append(history.history['loss'][0])
        non_zero_mask = y_true_matrix_test.swapaxes(0, 1)[1].sum(axis=1) > 0
        results = model.evaluate(train_bias_matrix[non_zero_mask], y_true_matrix_test[non_zero_mask], verbose=0)
